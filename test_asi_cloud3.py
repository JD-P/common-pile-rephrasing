...
def extract_rephrased_text(response_text):
    ...
    match = re.search(r'<rephrase>\n?(.*?)\n?</rephrase>', response_text, re.DOTALL)
    if match:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for template in TEMPLATES:
            # Load few-shot examples specific to this template type
            few_shot_messages = load_few_shot_examples(template["prefix"])
            
            if few_shot_messages:
                print(f"Loaded {len(few_shot_messages)} few-shot messages for {template['name']}")
            else:
                print(f"No few-shot examples loaded for {template['name']}, using zero-shot prompting")
